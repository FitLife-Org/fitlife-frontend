...
import type { BodyMetric, BodyMetricProgress } from "../../user/types/member.type";
...
export const bodyMetricService = {
  ...
  createMetricForMember: async (data: import("../../user/types/member.type").BodyMetricCreateRequest): Promise<BodyMetric> => {
    const response = await apiClient.post<ApiResponse<BodyMetric>>(`/admin/body-metrics`, data);
    return response.data.data;
  },

  updateMetric: async (id: number, data: import("../../user/types/member.type").BodyMetricUpdateRequest): Promise<BodyMetric> => {
