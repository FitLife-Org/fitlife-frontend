...
export default function EquipmentManagementPage() {
  ...
  const renderStatusBadge = (status: Equipment["status"]) => {
    switch (status) {
      case "ACTIVE":
        return <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium bg-emerald-50 text-emerald-600 border border-emerald-200"><span className="w-1.5 h-1.5 rounded-full bg-emerald-500"></span>Hoạt động</span>;
      case "MAINTENANCE":
        return <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium bg-amber-50 text-amber-600 border border-amber-200"><span className="w-1.5 h-1.5 rounded-full bg-amber-500"></span>Bảo trì</span>;
      case "INACTIVE":
        return <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium bg-rose-50 text-rose-600 border border-rose-200"><span className="w-1.5 h-1.5 rounded-full bg-rose-500"></span>Ngừng hoạt động</span>;
    }
  };
        ...
        <div className="flex items-center gap-3">
          <div className="relative">
            ...
            <input 
              ...
              className="pl-9 pr-4 py-2 bg-white border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 w-64 shadow-sm"
            />
          </div>
          ...
          <Button className="bg-emerald-600 hover:bg-emerald-700 text-white shadow-sm flex items-center gap-2 px-4 py-2 rounded-lg font-medium text-sm">
      ...
      <Card className="shadow-sm border-slate-100">
        ...
        <div className="p-5 border-b border-slate-100 flex flex-wrap lg:flex-nowrap items-center justify-between gap-4">
          <div className="w-full lg:w-80 relative">
            ...
            <input 
              ...
              className="w-full pl-9 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-colors"
            />
          </div>
          ...
          <div className="flex items-center gap-3 w-full lg:w-auto overflow-x-auto pb-1 lg:pb-0">
            ...
            <div className="flex flex-col gap-1 justify-end mt-[18px]">
              <button className="flex items-center gap-2 px-4 py-2 bg-white border border-slate-200 rounded-lg text-slate-700 hover:bg-slate-50 text-sm font-medium shadow-sm h-[38px] whitespace-nowrap">
                ...
                <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 absolute top-1 right-1 sm:static"></div>
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm text-slate-600">
            <thead className="text-xs uppercase text-slate-500 bg-slate-50/80 border-b border-slate-100 font-medium">
              <tr>
                <th className="px-5 py-4 w-12"><input type="checkbox" className="rounded border-slate-300 text-emerald-600 focus:ring-emerald-500 cursor-pointer" /></th>
            ...
            <tbody className="divide-y divide-slate-50">
              {MOCK_EQUIPMENTS.map((eq) => (
                <tr key={eq.id} className="hover:bg-slate-50/50 transition-colors group">
                  <td className="px-5 py-4"><input type="checkbox" className="rounded border-slate-300 text-emerald-600 focus:ring-emerald-500 cursor-pointer" /></td>
                    ...
                    {eq.nextMaintenance ? (
                      <div className="flex flex-col">
                        ...
                        {eq.daysToNextMaintenance !== null && (
                          <span className={`text-[11px] mt-0.5 ${eq.daysToNextMaintenance <= 7 ? 'text-amber-600 font-medium' : 'text-emerald-600'}`}>
                  ...
                  <td className="px-5 py-4">
                    <div className="flex items-center justify-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button className="p-1.5 text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 rounded-md transition-colors" title="Xem chi tiết">
                        <Eye className="w-4 h-4" />
                      </button>
                      <button className="p-1.5 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-md transition-colors" title="Chỉnh sửa">
        ...
        <div className="p-5 border-t border-slate-100 flex items-center justify-between">
          <div className="flex items-center gap-3 text-sm text-slate-600">
            <span>Hiển thị</span>
            <select className="px-2.5 py-1 border border-slate-200 rounded text-slate-700 bg-white focus:outline-none focus:border-emerald-500 cursor-pointer">
              <option>10</option>
              <option>20</option>
              <option>50</option>
            </select>
          ...
          <div className="flex items-center gap-1">
            ...
            <button className="w-8 h-8 rounded bg-emerald-600 text-white font-medium flex items-center justify-center text-sm shadow-sm hover:bg-emerald-700 transition-colors">1</button>
