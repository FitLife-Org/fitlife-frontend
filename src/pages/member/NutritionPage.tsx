...
function buildDetailRoute(id: number): string {
    return ROUTES.MEMBER_NUTRITION_DETAIL.replace(
        ":id",
        String(id),
    );
}
...
export default function NutritionPage() {
    ...
    const loadPlans = useCallback(async () => {
        try {
            ...
            const page = await nutritionService.getMyPlans(0, 20);
    ...
    const handleActivate = async (
        ...
        try {
            ...
            await nutritionService.activatePlan(plan.id);

            toast.success("Đã kích hoạt kế hoạch dinh dưỡng.");
            await loadPlans();
        } catch (requestError) {
            toast.error(
                getApiErrorMessage(
            ...
            {loading ? (
                ...
                <Card className="p-10 text-center">
                    ...
                    <p className="mx-auto mt-2 max-w-lg text-sm leading-6 text-slate-500">
                        Hãy sử dụng FitLife AI để tạo kế hoạch calories,
                        macro và bữa ăn phù hợp.
                    </p>
                    <Button
                        variant="primary"
                        onClick={() => navigate(ROUTES.MEMBER_AI)}
                        ...
                        <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                            ...
                            <Card className="p-5">
                                ...
                                <p className="mt-2 text-2xl font-black text-blue-600">
                                    {activePlan.carbohydrateGrams ?? "—"} g
                                </p>
                            </Card>
                          ...
                          {getStatusLabel(plan.status)}
                                        ...
                                        <div className="mt-5 grid grid-cols-2 gap-3 text-sm">
                                            ...
                                            <div className="rounded-xl bg-slate-50 p-3">
                                                ...
                                                <p className="mt-1 font-bold text-slate-700">
                                                    {plan.mealsPerDay ?? plan.meals.length} bữa/ngày
                                                </p>
                                            </div>
                                        ...
                                        <div className="mt-5 flex flex-wrap gap-2">
                                            ...
                                            <Button
                                                variant="outline"
                                                onClick={() =>
                                                    navigate(buildDetailRoute(plan.id))
                                                }
                                            >
                                                Xem chi tiết
